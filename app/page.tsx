...
const memberProfiles = [
  {name:'Chipo Nyambuya, Esq',role:'International Legal, Governance, and Economic Development Leader',bio:'Her areas of leadership bring together international law, governance and economic development.',expertise:['International law','Governance','Economic development'],image:'/assets/Smiling shaved-head portrait with colorful jewelry.png'},
  {name:'Elizabeth L. Carter, Esq',role:'Business and Corporate Securities Attorney, Legal & Business',bio:'Her work connects business law, corporate securities and the legal foundations that support enterprise.',expertise:['Business law','Corporate securities','Legal & business'],image:'/assets/Red-bloused portrait on a warm ivory backdrop.png'},
  {name:'Priscilla Cadette',role:'Entrepreneur, mentor and fundraising specialist',bio:'Her experience connects entrepreneurship, mentorship and fundraising support.',expertise:['Entrepreneurship','Mentorship','Fundraising'],image:'/assets/Yellow headwrap portrait with gold neck rings.png'}
] as const;
const regions = {
...
const sectorStories: Record<Sector,{image:string;alt:string;kicker:string;description:string}> = {
  ...
  'Consulting Services': {
    image:'/assets/member-consulting.jpg',
  ...
  Agriculture: {
    image:'/assets/member-agriculture.jpg',
  ...
  'Textiles & Apparel': {
    image:'/assets/member-textiles.jpg',
  ...
  'Handmade Crafts': {
    image:'/assets/member-crafts.jpg',
  ...
  Training: {
    image:'/assets/member-training.jpg',
...
export default function Home(){
  ...
  return <MotionConfig reducedMotion="user"><main className={`${styles.page} min-h-screen`}>
    ...
    <section className="section global-section"><div className="shell global-grid"><div className="global-copy"><p className="eyebrow">Local roots. Global reach.</p><h2>The orbit becomes the story.</h2><p>WisConnect’s network is rooted in Africa and extends toward relationships and opportunities across regions.</p><div className="region-buttons">{(Object.keys(regions) as Region[]).map(r=><button key={r} className={region===r?'active':''} onClick={()=>setRegion(r)}>{r}</button>)}</div><div className="region-detail"><strong>{region}</strong><p>{regions[region]}</p></div></div><div className="global-orbit"><div className="ring ring-1"></div><div className="ring ring-2"></div><div className="ring ring-3"></div><img className="orbit-logo" src="assets/logo-symbol.webp" alt="WisConnect orbit symbol"/><span className="node node-us">US</span><span className="node node-br">BR</span><span className="node node-vn">VN</span><span className="node node-af">AF</span></div></div></section>
